...
utils.ping = function(host, cb) {
  ...
  module.exports.isLocalAsync(host)
  .then(r => {
    //module.exports.isLocal(host)
    if(!r) {
...
utils.checkPort = function(server, cb) {
  ...
  generateCommand(self, host, port)
  .then(cmd1 => {
...
//
utils.isLocalAsync = function(host) {
  var app = require('../pomelo').app;
  if(!app) {
    var ret = host === '127.0.0.1' || host === 'localhost' || host === '0.0.0.0';

    if(ret){
      return Promise.resolve(ret);
    }
    else{
      return inLocalAsync(host);
    }

  } else {
    var ret = host === '127.0.0.1' || host === 'localhost' || host === '0.0.0.0'; // || inLocal(host) || host === app.master.host;
...
var inLocalAsync = function(host) {
  return dnsResolve(host)
  .then( (ip) => {
    for (var index in localIps) {
      if (ip === localIps[index]) {
        return true;
      }
    }
    return false;
...
var dnsResolve = function (host) {
  return new Promise((resolve,reject) => {
    dns.lookup(host,(err,addr) => {
      if(!err){
        logger.warn(host,' dns lookup ',addr);
